...
class LlamaStackEmbeddingsWrapper(BaseRagasEmbeddings):
    ...
    def embed_query(self, text: str) -> List[float]:
        """Synchronous embed query using Llama Stack inference API."""
        try:
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        """Synchronous embed documents using Llama Stack inference API."""
        try:
            loop = asyncio.get_event_loop()
    ...
    async def aembed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            response = await self.inference_api.embeddings(
                ...
                contents=texts,
                task_type="document",  # For asymmetric models
            )
    ...
    async def aembed_query(self, text: str) -> List[float]:
        ...
        try:
            response = await self.inference_api.embeddings(
                model_id=self.embedding_model_id,
                contents=[text],
                task_type="query",  # For asymmetric models
            )
...
class LlamaStackLLMWrapper(BaseRagasLLM):
    ...
    def generate_text(
        ...
        """Synchronous text generation using Llama Stack inference API."""
        try:
            ...
            for _ in range(n):
                # Use asyncio to run the async completion in sync context
                response = asyncio.run(
                    self.inference_api.completion(
                        model_id=self.model_id,
                        content=prompt_text,
